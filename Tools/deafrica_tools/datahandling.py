    ...
    **kwargs : dict, optional
    ...
    if product_type == "ls":
        if any(k in categories_to_mask_ls for k in ("cirrus", "cirrus_confidence")):
            raise ValueError(
                "'cirrus' categories for the pixel quality mask" " are not supported by load_ard"
            )
    ...
    data_bands = [
        band for band in measurements if band not in (fmask_band, "radiometric_saturation")
    ]
    ...
    for product in products:
        ...
        if not ls7_slc_off and product in ["ls7_sr"]:
            ...
            datasets = [
                i for i in datasets if i.time.begin < datetime(2003, 5, 31, tzinfo=pytz.UTC)
            ]
...
def load_s1_by_orbits(dc, query):
    ...
    ds_s1_ascending["is_ascending"] = xr.DataArray(
        np.ones(len(ds_s1_ascending.time)), dims=("time"), coords={"time": ds_s1_ascending.time}
    )

    # load descending data
    print("\nQuerying and loading Sentinel-1 descending data...")
    ds_s1_descending = load_ard(
        dc=dc,
        products=["s1_rtc"],
        resampling="bilinear",
        dtype="native",
    ...
    # add an variable denoting data source
    ds_s1_descending["is_ascending"] = xr.DataArray(
        np.zeros(len(ds_s1_descending.time)), dims=("time"), coords={"time": ds_s1_descending.time}
    )
...
def filter_obs_by_orbit(ds_s1):
    ...
    ds_s1_filtered = ds_s1.where(
        ((cnt_ascending >= cnt_descending) & (ds_s1["is_ascending"] == 1))
...
def lee_filter(da, size):
    ...
    overall_variance = scipy.ndimage.variance(img)
...
def create_coastal_mask(da, buffer_pixels):
    ...
    coastal_mask = (thresholded_ds.mean(dim="time") >= 0.2) & (
        thresholded_ds.mean(dim="time") <= 0.8
    )
    # buffering
    print(
        "\nApplying buffering of {} Sentinel-2 pixels (parameter buffer_pixels)...".format(
            buffer_pixels
        )
    )
    coastal_mask = xr.apply_ufunc(
        skimage.morphology.binary_dilation,
        coastal_mask.compute(),
...
def choose_product(ds_ls, ds_s2, ds_s1, ds_ls_s2, time_step, **kwargs):
    ...
    **kwargs: A set of optional parameters including:
    ...
    print(
        "\nThreshold number of valid observations (parameter thresh_n_valid): {}".format(
            thresh_n_valid
        )
    )
    print(
        "\nThreshold frequency of valid observations (parameter thresh_freq): {}".format(
            thresh_freq
        )
    )
    ...
    print(
        "\nSentinel-2: Average number and frequency of valid observations: {:.0f} and {:.2f}".format(
            n_valid_obs_s2.mean().values, freq_valid_s2.mean().values
        )
    )

    n_valid_obs_ls, freq_valid_ls = get_mean_number_freq_valid_obs(ds_ls["green"], mask, time_step)
    print(
        "\nLandsat: Average number and frequency of valid observations: {:.0f} and {:.2f}".format(
            n_valid_obs_ls.mean().values, freq_valid_ls.mean().values
        )
    )
    ...
    if ds_ls_s2 is not None:
        ...
        print(
            "\nCombined Landsat and Sentinel-2 product: Average number and frequency of valid observations: {:.0f} and {:.2f}".format(
                n_valid_obs_ls_s2.mean().values, freq_valid_ls_s2.mean().values
            )
        )
    ...
    if ((n_valid_obs_s2 >= thresh_n_valid).all()) and ((freq_valid_s2 >= thresh_freq).all()):
        ...
        elif ((n_valid_obs_ls >= n_valid_obs_s2).all()) and (
            (freq_valid_ls >= freq_valid_s2).all()
        ):
    ...
    # if neither Sentinel-2 or Landsat meet both requirements, choose combined product if it meets requirements
        ...
        if ((n_valid_obs_ls_s2 >= thresh_n_valid).all()) and (
            (freq_valid_ls_s2 >= thresh_freq).all()
        ):
            ds_selected, product_name = ds_ls_s2, "ls_s2"
            print(
...
def load_combined_ls_s2(dc, query):
    ...
    ds_ls = load_ard(
        dc=dc, products=["ls8_sr", "ls9_sr"], align=(10, 10), resampling="bilinear", **query
    )
...
def load_best_available_ds(dc, lat_range, lon_range, time_range, time_step, **kwargs):
    ...
    **kwargs: A set of optional parameters on data query or comparison between products which may include:
    ...
    min_time = min(
        parser.parse(time_range_i, default=datetime(1987, 1, 1, 0, 0))
    ...
    if "set_resolution" not in kwargs:
        ...
        resolution_ls = resolution_s2 = resolution_s1 = (
            kwargs["set_resolution"] * (-1),
            kwargs["set_resolution"],
        )
    ...
    if set_product == "ls":
        ...
        ds_selected = load_ard(dc=dc, products=["ls8_sr", "ls9_sr"], resampling="bilinear", **query)
    elif set_product == "s2":
        print("\nPre-selected product: Sentinel-2")
        if ls_only:
            raise ValueError(
                "Querying date earlier than 2018, please change your pre-selected product as Landsat or query time range."
            )
        query.update({"resolution": resolution_s2})
        ds_selected = load_ard(
            dc=dc,
            products=["s2_l2a"],
            resampling="bilinear",
            mask_filters=[("opening", 2), ("dilation", 5)],
        ...
        if ls_only:
            ...
            ds_ls = load_ard(dc=dc, products=["ls8_sr", "ls9_sr"], resampling="bilinear", **query)
            ds_selected = ds_ls
            product_name = "ls"
        else:
            ...
            ds_selected, product_name = choose_product(
                ds_ls, ds_s2, ds_s1, ds_ls_s2, time_step, **kwargs
            )
