...
def WIT_drill(
    ...
    print(
        "working on polygon: " + str(gdf_poly.drop("geometry", axis=1).values) + ".  "
    )

    # make quaery from polygon
    geom = geometry.Geometry(
        gdf_poly.geometry.values[0].__geo_interface__, geometry.CRS("epsg:4326")
    )
    ...
    print("size of wetlands array: " + str(ls578_ds.isel(time=1).red.values.shape))
