...
def add_geobox(ds: xr.Dataset | xr.DataArray, crs: str | None = None) -> xr.Dataset | xr.DataArray:
    """
...
def subpixel_contours(
    da: xr.DataArray,
    z_values: int | float | list[int] | list[float] = [0.0],
    crs: str | None = None,
    attribute_df: pd.DataFrame | None = None,
    output_path: str | None = None,
    min_vertices: int = 2,
    dim: str = "time",
    ...
    if len(da.shape) == 2:
        ...
        contour_arrays = {
            _time_format(i, time_format): _contours_to_multiline(da_i, z_values[0], min_vertices)
...
def contours_to_arrays(gdf: gpd.GeoDataFrame, col: str) -> np.ndarray:
    """
...
def largest_region(bool_array: np.ndarray, **kwargs) -> np.ndarray:
    """
...
def transform_geojson_wgs_to_epsg(geojson: dict, EPSG: int) -> dict:
    """
...
def sun_angles(dc: Datacube, query: dict) -> xr.Dataset:
    """
