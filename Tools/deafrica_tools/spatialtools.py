...
def xr_vectorize(
    ...
    **rasterio_kwargs :
    ...
    if type(da) is np.ndarray:
        ...
        vectors = rasterio.features.shapes(
            source=da.data.astype(dtype), transform=transform, **rasterio_kwargs
        )
...
def xr_rasterize(
    ...
    **rasterio_kwargs :
    ...
    xarr = xr.DataArray(
        arr, coords=xy_coords, dims=dims, attrs=da.attrs, name=name if name else None
    )
...
def subpixel_contours(
    ...
    if output_path and output_path.endswith(".geojson"):
        ...
        contours_gdf.to_crs({"init": "EPSG:4326"}).to_file(
            filename=output_path, driver="GeoJSON"
        )
...
def interpolate_2d(
    ds, x_coords, y_coords, z_coords, method="linear", factor=1, **kwargs
):
    ...
    for i in range(0, len(gdf)):
        ...
        try:
            coords = np.concatenate(
                [np.vstack(x.coords.xy).T for x in gdf.iloc[i].geometry]
            )
        except:
