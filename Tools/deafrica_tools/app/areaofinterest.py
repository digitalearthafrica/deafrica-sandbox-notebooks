...
class upload_aoi(HBox):
    def __init__(self):
        ...
        fileupload_html = deawidgets.create_html(value=f"""<h2>Upload a GeoJSON or ESRI Shapefile</h2></br><i></br>Upload a GeoJSON or ESRI Shapefile (<5 mb) containing a single area of interest. When uploading an ESRI shapefile, ensure that all shapefile components (e.g. .shp, .shx, .dbf, .prj) are uploaded.</i>""")
