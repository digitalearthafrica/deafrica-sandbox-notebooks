...
from ipyleaflet import WMSLayer, basemaps, basemap_to_tiles, Map, DrawControl, WidgetControl, LayerGroup, LayersControl, GeoData
...
def load_gfclayer(self):
    ...
    min_lat, max_lat = self.gdf_drawn.bounds.miny.item(), self.gdf_drawn.bounds.maxy.item()
    min_lon, max_lon = self.gdf_drawn.bounds.minx.item(), self.gdf_drawn.bounds.maxx.item()
    ...
    ds = ds.rio.clip_box(minx=min_lon-0.00025, miny=min_lat-0.00025, maxx=max_lon+0.00025, maxy=max_lat+0.00025)
...
def get_basemap(basemap_url):
    ...
    elif basemap_url == 'http://server.arcgisonline.com/ArcGIS/rest/services/World_Imagery/MapServer/tile/{z}/{y}/{x}':
...
def plot_gfclayer(self):
    ...
    if self.gfclayer == "lossyear":
        # Create the basemap. 
        plot_basemap = get_basemap(self.basemap.url).opts(height=500, 
                                                          width=700)
        
        # Pass the ds xarray.Dataset to hv.Dataset
        # to create and object called "dataset."
        dataset = gv.Dataset(data=self.gfclayer_ds,
        ...
        image = dataset.to(gv.Image).opts(colorbar=True,
                                          cmap=color_list,
...
class forest_monitoring_app(HBox):
    def __init__(self):
        ...
        self.basemap_list = [("Open Street Map", basemap_to_tiles(basemaps.OpenStreetMap.Mapnik)),
        ...
        dropdown_basemap = deawidgets.create_dropdown(options=self.basemap_list,
                                                      value=self.basemap)
        ...
        self.gfclayers_list = [("Year of gross forest cover loss event", "lossyear"),
        ...
        self.gfclayer_ds = None
        # Dropdown selection widget. 
        dropdown_gfclayer = deawidgets.create_dropdown(options=self.gfclayers_list,
                                                      value=self.gfclayer)
        # Register the update function to run when a new value is selected 
        ...
        self.max_size = False
        # CheckBox widget. 
        checkbox_max_size = deawidgets.create_checkbox(value=self.max_size, 
                                                       description="Enable", 
                                                       layout={"width": "95%"})
        # Text to accompany the CheckBox widget.
        checkbox_max_size_html = deawidgets.create_html(value=f"""</br><b>Override maximum size limit: 
        ...
        parameter_selection = VBox([basemap_selection, 
                                    gfclayer_selection, 
                                    enable_max_size])
        ...
        run_button = create_expanded_button(description="Generate plot", 
                                            button_style="info")
    ...
    def run_app(self, change):
        ...
        if self.gdf_drawn is not None:
            ...
            if self.gfclayer_ds is not None:
                ...
                with self.status_info:
                    print(f"""No Global Forest Change {self.gfclayer} layer 
        ...
        # If no valid polygon was drawn.
            ...
            with self.status_info: 
                print('Please draw a valid polygon on the map, then click on "Geneate plot"')